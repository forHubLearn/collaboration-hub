...
export function printReceipt(tx: Transaction, storeName = 'BuildMat Store') {
  ...
  const itemsHtml = tx.items.map(item => {
    const taxLines = item.taxes.map(t =>
      `<tr style="font-size:11px;color:#666"><td style="padding-left:12px">${t.name} (${t.percentage}%)</td><td style="text-align:right">Br${t.amount.toFixed(2)}</td></tr>`
    ).join('');
    return `
      <tr>
        <td>${item.materialName}</td>
        <td style="text-align:right">Br${item.unitPrice} × ${item.quantity}</td>
      </tr>
      ${taxLines}
    `;
  }).join('');
  ...
  const html = `
    ...
    <body>
      ...
      <div class="divider"></div>
      <table>
        <tr><td>Subtotal</td><td style="text-align:right">Br${tx.subtotal.toFixed(2)}</td></tr>
        <tr><td>Tax</td><td style="text-align:right">Br${tx.totalTax.toFixed(2)}</td></tr>
        <tr class="total-row"><td>TOTAL</td><td style="text-align:right">Br${tx.totalPrice.toFixed(2)}</td></tr>
      </table>
      <div class="divider"></div>
